...
class FaqCateForm extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      isFather:
        this.props.selRecord === undefined
          ? true
          : this.props.selRecord.level === 0
          ? true
          : false,
      selUserOptions:
        this.props.selRecord === undefined
          ? []
          : this.props.selRecord.specialUser_access || [],
  ...
  initialSelGroupOptions = () => {
    ...
    let selGroupOptions = groupOptions.filter((item) =>
      selRecord.specialGroup_access.find((sel) => sel === item.id)
    );
    this.setState({
      selGroupOptions: selGroupOptions.map((iGroup) => (
        <Option key={iGroup.id}>{iGroup.groupName}</Option>
      )),
    });
  };
      ...
      selReader.forEach((iReaderId, index, array) => {
        fetchData(getARbyId_header + iReaderId, "get", null, (response) => {
          ...
          if (index === array.length - 1) {
            ...
            this.setState({
              ruleOptions: selReaderArray,
              selRuleOptions: selReaderArray.map((item) => (
  ...
  onSearchUser = (inputs) => {
    if (inputs) {
      let getUserByStaffNo_url =
        sessionStorage.getItem("serverPort") +
        "user/getUserByStaffNo/" +
        sessionStorage.getItem("@userInfo.id") +
        "?staffNo=" +
        inputs;
    ...
    this.props.form.validateFieldsAndScroll((err, fieldsValues) => {
      if (!err) {
        ...
        const values = {
          ...
          ...fieldsValues,
          titleTc:
            fieldsValues["titleTc"] === null || fieldsValues["titleTc"] === ""
              ? fieldsValues["titleEn"]
              : fieldsValues["titleTc"],
          level:
            fieldsValues["parentForumId"] === null ||
            fieldsValues["parentForumId"] === 0
              ? 1
              : 2,
    ...
    return (
      <div>
          ...
          <span hidden={selRecord.level === 0}>
            ...
            <Form.Item label={intl.get("@FAQ_ADMIN.ANSWER-TC")}>
              {getFieldDecorator("AnswerTc", {
                initialValue: selRecord.answerTc,
                valuePropName: "data",
                getValueFromEvent: this.onEditorChange,
              })(
                // <textarea  style={{minWidth: '500px'}} allowClear />
        ...
        >
          ...
          >
            <Button type="danger" disabled={submitting} loading={deleting}>
