...
class PostForm extends Component{
    ...
    getCreatorsList=()=>{
        let getBlogger_url = sessionStorage.getItem('serverPort')+`blog/assistant/getOriginal/${sessionStorage.getItem('@userInfo.id')}`;
        fetchData(getBlogger_url,'get',null,response=>{
            if(response.ifSuccess){
              let res = response.result;
              if(res.status===200&&res.data){
                  this.setState({ postCreatorsList: res.data.sort((a,b) => (a.fullname > b.fullname) ? 1 : ((b.fullname > a.fullname) ? -1 : 0)) });
              }
            }
        });
    }
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err,fieldsValues)=>{
            if(fieldsValues['nominatedAuthor']==='alias'&&!radioInput){
                ...
                let createPost_url = sessionStorage.getItem('serverPort')+'blog/create/'+sessionStorage.getItem('@userInfo.id');
                fetchData(createPost_url, 'post', values, response=>{
                    ...
                    if(response.ifSuccess){
                      ...
                      if(res.status===200){
                          ...
                          setTimeout(()=> { window.location.replace(`#/miniblog/details/?id=${res.data}`) }, 1000);
                      }else{
    ...
    onDelete = () => {
        window.location.assign('#/miniblog/myblog');
    }
    ...
    render() {
        ...
        const { username, categoryList, mode, disableTimePicker, radioValue, postCreatorsList, loading, isSuperAdmin } = this.state;

        const radioStyle1 = { display: 'inline', height: '30px', lineHeight: '30px', };
        const radioStyle2 = { display: 'block', height: '30px', lineHeight: '30px', };
        ...
        <div>
            ...
            <Form layout="horizontal" labelAlign="left" onSubmit={this.handleSubmit}>
                ...
                {getFieldDecorator('categoryId', {
                    ...
                    <Select maxMenuHeight={100} className="postTags" placeholder={intl.get('@MINI_BLOG.CATEGORY-TIPS')} >
...
{/* ---------------------------------------------------------------------------- */}
                ...
                {getFieldDecorator('nominatedAuthor',{ initialValue: "user" })(
                    <RadioGroup name="nominatedAuthor" onChange={this.onRadioChange}>
                        <Radio style={radioStyle1} value="user">{username}</Radio>
                        <Radio style={radioStyle1} value="alias">
                            {intl.get('@MY_BLOG.ALIAS')}
                            {radioValue === "alias" ? <Input onChange={this.onRadioInputChange} style={{ maxWidth: '16em', marginLeft:10, lineHeight: 0, display: 'inline-block' }} allowClear /> : null}
                        </Radio>
                        
                        <Radio style={radioStyle2} disabled={postCreatorsList.length===0} value="others">
                            ...
                            {radioValue === "others" ? 
                                ...
                                >
                                    {postCreatorsList.map(option=>{return <Option key={option.id} label={`${option.fullname}(${option.staffNo})`}>{`${option.fullname}(${option.staffNo})`}</Option>})}
                                </Select>
                                : null
                            }
                        </Radio>
