...
class userMntForm extends React.Component{
  ...
  handleAvatarApprove = (type, approve,id) => {
    let data = {
    ...
    // let approve_url = sessionStorage.getItem('serverPort') + "user/approve/" + sessionStorage.getItem('@userInfo.id') + "/"+ type+"/"+approve+"/"+id;
  ...
  render(){
    ...
    const approveCustomAvatar = (type, loading,id) => {
      ...
      return <Button.Group style={{ textAlign: 'center', width: '100%', marginTop: '1.5rem' }}>
        <Button type="primary" loading={loading} onClick={(e)=>this.handleAvatarApprove(type, 1,id)} >{intl.get('@GENERAL.APPROVE')}</Button>
        <Button type="danger" loading={loading} onClick={(e)=>this.handleAvatarApprove(type, 0,id)} >{intl.get('@GENERAL.REJECT')}</Button>
      </Button.Group>
    }
    ...
    return(
      ...
      <div>
        <div className="user-photo">
          ...
          >
            {imageUrl_profile ? <img src={imageUrl_profile} className="user-photo-uploaded" alt="profile" /> : null}
          ...
          {approveCustomAvatar(1, loading_profile,id)}
        </div>
        ...
        <Form layout="vertical" labelCol={{ xs:{span:64}, sm:{span:6} }} wrapperCol={{ xs:{span:24}, sm:{span:18} }} labelAlign="left" >
          ...
          <div className="user-photo" style={{ marginBottom: '12px' }}>
            ...
            >
              {imageUrl_alias ? <img src={imageUrl_alias} className="user-photo-uploaded" alt="alias" /> : null}
              {/* {imageUrl_alias ? <img is="auth-img" authSrc={imageUrl_alias} className="user-photo-uploaded" alt="alias" /> : uploadButton(loading_alias)} */}
            </Upload>
            {approveCustomAvatar(2, loading_alias,id)}
          </div>
