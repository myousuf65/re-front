...
export default class PostEditor extends React.Component{
    ...
    componentDidMount(){
        ...
        let getLatest_url = sessionStorage.getItem('serverPort')+'blog/getLatest/'+sessionStorage.getItem('@userInfo.id')+'/0';  //---1 for create log in backend; 0 for not;
        ...
        let intl_url = sessionStorage.getItem('serverPort')+'blog/getDetail/'+post_id+'?user='+sessionStorage.getItem('@userInfo.id');
            ...
            <div className="page-content">
                <div className="container create-post">
                    ...
                    <div className="col-lg-3">
                        ...
                        <h2><span style={{ backgroundImage:'images/icon-from-the-blog.png'}}>{intl.get('@MY_BLOG.MOST-RECENT')}</span></h2>
                        <ul className="related-blog-list" style={{ marginBottom: 0 }}>
                            {this.state.relatedList.map(item=>{
                                return (
                                    <li key={typeof(item.blog)==='undefined'? 0:item.blog.id}>
                                    {/* eslint-disable-next-line */}
                                    <a href={`#/miniblog/details/?id=${typeof(item.blog)==='undefined'? 0:item.blog.id}`}>{typeof(item.blog)==='undefined'? '':item.blog.postTitle}</a>
                                    </li>
                                )
                            })}
                        </ul>
                        ...
                        <div className="related-resources" >
                            ...
                            <List
                                ...
                                header={<b>{intl.get('@MY_BLOG.ARCHIVED')}</b>}
                                bordered
