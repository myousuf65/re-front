...
const loadingImg =
  process.env.PUBLIC_URL + "/images/elearning-img-loading.jpeg";
...
class QuestionFormEdit extends React.Component {
  state = {
    ...
    uploadList: [],
    submitting: false,
    selRecord: this.props.selRecord,
    catAllList: this.props.catAllList,
    fileList: [],
    ruleOptions: [],
    category: [],
    categoryList: [],
  ...
  componentDidMount() {
    ...
    this.setState({
      ans_num: selRecord.answer.split(";,;").length,
      correctAns: selRecord.correctAnswer,
      randomSetting: selRecord.randomSetting,
    });
  ...
  initialGalleryList = () => {
    let checkGallery_url =
      sessionStorage.getItem("serverPort") +
      `elearning/gallery/check/${sessionStorage.getItem("@userInfo.id")}`;
      ...
      if (response.ifSuccess) {
        ...
        if (res.fileList && res.msg !== "Gallery Created") {
          ...
          res.fileList.forEach((item) => {
            ...
            fetchData(
              sessionStorage.getItem("serverPort") + item.ofilename,
              "get",
              null,
              (response) => {}
            );
    ...
    imgArr.forEach((img) => {
      ...
      if (!imgOriginAlt.startsWith("resources/") && imgOriginSrc) {
        if (imgOriginSrc.startsWith("https://dsp.csd.hksarg/kms/api/")) {
          ...
          if (imgId > -1) {
            let newKMSImg = `resources/${selPost.createdby}/${
              selPost.id
  ...
  handleDraggerRemove = (file) => {
    if (typeof file.response !== "undefined") {
      if (
        typeof file.response.data !== "undefined" &&
      ...
      answerInputs.push(
        ...
        >
          {getFieldDecorator(fieldName, {
            ...
            rules: [{ required: true, message: `Answer ${i} is required` }],
          })(
            <Input
              min={2}
              max={8}
              style={{ lineHeight: 0, maxWidth: "800px" }}
              placeholder={`Answer ${i}`}
            />
          )}

          <Radio value={i}>
            {sessionStorage.getItem("lang") === "zh_TW" ? "答案" : "Answer"}
          </Radio>
        </Form.Item>
      );
    }
  ...
  render() {
    ...
    let draggerprops = {
      ...
      action:
        sessionStorage.getItem("serverPort") +
        `elearning/gallery/upload/${sessionStorage.getItem("@userInfo.id")}`,
      headers: httpHeaders,
        ...
        <div className="row">
          <div className="col-lg-9">
            ...
            >
              ...
              >
                {getFieldDecorator("question", {
                  ...
                  rules: [
                    {
                      required: true,
                      message: intl.get("@GENERAL.IS-REQUIRED"),
                    },
                  ],
              ...
              <Form.Item label={intl.get("@E_LEARNING.VIDEO_LINK")}>
                {getFieldDecorator("post_video_link", {
                  ...
                  rules: [
                    {
                      required: false,
                      message:
                        intl.get("@E_LEARNING.VIDEO_LINK") +
                        " " +
                  ...
                  <Input
                    ...
                    allowClear
                    placeholder={intl.get("@E_LEARNING.VIDEO_LINK")}
                  />
                )}
              </Form.Item>
              ...
              <Form.Item label={intl.get("@E_LEARNING.RANDOM")}>
                {getFieldDecorator("randomSetting", {
                  ...
                  <Radio.Group
                    name="rnd_ans"
                    onChange={this.handleRandomAnswerChange}
                  >
          ...
          <div className="col-lg-3">
            <div className="related-resources">
              <h2>
                <span
                  style={{
                    backgroundImage: "images/icon-from-the-blog.png",
                    color: "gold",
                  }}
                >
                  <b>{intl.get("@MY_BLOG.GALLERY-MANAGER")}</b>
                </span>
              </h2>
              <div className="gallery-manager" style={{ minHeight: "auto" }}>
                ...
                <List
                  ...
                  bordered
                  dataSource={this.state.fileList.sort((a, b) =>
                    ...
                    <List.Item
                      ...
                      extra={
                        <img
                          ...
                          disabled={true}
                          onClick={(e) =>
                            this.handleImgSrc(item.ofilename, e.target)
                          }
                          alt={item.ofilename}
                          data={item.ofilename}
                        />
...
const elearningQuestionFormEdit = Form.create({ name: "quiz_form" })(
  QuestionFormEdit
);
