...
export default class ElearningQuestionManage extends Component{
    ...
    handleAddQuestion = (id) => {
        ...
        const selectedQuestion = add_questions.find(question => question.id === id);
    
        if (selectedQuestion) {
          this.setState(prevState => ({
            questions: [...prevState.questions, selectedQuestion],
            add_questions: prevState.add_questions.filter(question => question.id !== id)
          }));
        }
    }
    ...
    render(){
        const { questions, add_questions,categoryOptions,searchQuery, newQuestionTitle, newQuestionAnswers, correctAnswer } = this.state;
                                    ...
                                    <div>
                                        ...
                                        <div className="scrollable-div">
                                            {displayQuestions.map(question => (
                                                <div key={question.id} className="question-add" style={{ display: 'flex', alignItems: 'center' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <label className="question-text">{question.text}</label>
                                                    </div>
                                                    <div>
                                                        <button className="btn-remove" onClick={() => this.handleRemoveQuestion(question.id)}>Remove</button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="searchable-div">
                                            <div className="search-bar">
                                                <div className="search">
                                                    ...
                                                    placeholder="Search"
                                                    />
                                                    <button className="btn-elearning-search" onClick={this.handleSearchQuestions}>Search</button>
                                                </div>
                                            ...
                                            {searchQuestions.map(question => (
                                                <div key={question.id} className="searched-question" style={{ display: 'flex', alignItems: 'center' }}>
                                                    <div style={{ flex: 1 }}>
                                                        <label className="question-text">{question.text}</label>
                                                    </div>
                                                    <div>
                                                        <button className="btn-add" onClick={() => this.handleAddQuestion(question.id)}>Add</button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="add-question-form">
                                            ...
                                            <div className="correct-answer">
                                                ...
                                                <button className="btn-elearning" onClick={this.handleAddQuestion}>Submit</button>
