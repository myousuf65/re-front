...
class BottomPostList extends React.Component{
    ...
    componentDidMount(){
        ...
        let intl_url = sessionStorage.getItem('serverPort')+'newscorner2/search/?user='+sessionStorage.getItem('@userInfo.id')+'&CateId=0&selSorter=publish_at:desc&page=1';
        fetchData(intl_url, 'get', null, response=>{
            if(response.ifSuccess){
                let res = response.result;
                if(res.status===200){
    ...
    handlePageChange = page => {
        ...
        let pagin_url = sessionStorage.getItem('serverPort')+"newscorner2/search/?user="+sessionStorage.getItem('@userInfo.id');
    ...
    handleFilterChange = (cateId) => {
        ...
        let filter_url = sessionStorage.getItem('serverPort')+"newscorner2/search/?user="+sessionStorage.getItem('@userInfo.id');
    ...
    handleSorterChange = (selSorter) => {
        ...
        let sorter_url = sessionStorage.getItem('serverPort')+"newscorner2/search/?user="+sessionStorage.getItem('@userInfo.id');
            ...
            <div>
                ...
                <div className="container blog-list">
                    ...
                    renderItem={item => (
                        ...
                        >
                            <List.Item.Meta
                            title={<a href={item.newsCorner2.link} target="_blank">{item.newsCorner2.postTitle} </a>}
                            description={
                            <label>
                                <span> 
                                {
                                    moment(item.newsCorner2.publishAt).format('YYYY-MM-DD')
                                }
                                </span>
                            </label>}
                            />
                            <div dangerouslySetInnerHTML={{ __html: typeof(item.blog)==='undefined'? '':this.stripHtml(item.newsCorner2.content) }} />
...
export default class NewsCornerHome extends React.Component{
    ...
    componentDidMount(){
        ...
        let intl_url = sessionStorage.getItem('serverPort')+'newscorner2/getLatest/'+sessionStorage.getItem('@userInfo.id')+'/1';  //---1 for create log in backend; 0 for not;
        ...
        return(
            <div>
                ...
                <div className="container clearfix">
                    <a href="#/newscorner/home"><h2>{intl.get('@NEWSCORNER.MINI-BLOG')}</h2></a>
                </div>
                </div>
