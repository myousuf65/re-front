...
export default class PostDetails extends React.Component{
    ...
    state={ 
        ...
        modalVisible: false,
        modalTitle: sessionStorage.getItem('lang')==="zh_TW"? termsOfUseJson.miniblog.titleTc:termsOfUseJson.miniblog.titleEn, 
        modalContent: sessionStorage.getItem('lang')==="zh_TW"? termsOfUseJson.miniblog.contentTc:termsOfUseJson.miniblog.contentEn, 
        rootCmntId: -1,
        rootCmnt: null,
        cmntAuthor: {},
        relatedPosts: []
    ...
    getSelPost=(selPostId)=>{
        ...
        let selpost_url= sessionStorage.getItem('serverPort')+'blog/getDetail/'+selPostId+'?user='+sessionStorage.getItem('@userInfo.id');
        ...
        if(window.location.host==='dsp.csd.hksarg'){
            ...
            let getFromEngine_url = sessionStorage.getItem('serverPort') + "search/related/" + sessionStorage.getItem('@userInfo.id') + "?resource_url=" +postUrl+"&return_type=blog";

            this.getRelatedPosts(getFromEngine_url);
        } 
        else{
            // ------for internet 
            let getFromServer_url = sessionStorage.getItem('serverPort') + "blog/related/" + sessionStorage.getItem('@userInfo.id') + "?post="+selPostId;
    ...
    handleGallery=(fileList)=>{
        ...
        preFileList.forEach(img=>{
            if(!img.ofilename){ blobFileList.push(img) }
    ...
    showModal = (rootCmnt, cmntAuthor) => {
        this.setState({ modalVisible: true, rootCmnt: rootCmnt, cmntAuthor: cmntAuthor });
    }
...
// ------edit a comment
    ...
    render() {
        const { loading, relatedPosts, hindCmntEditor, hindCmntUpdater } = this.state;
        ...
        return(
            ...
            <div className="container clearfix">
                <a href="#/miniblog/home"><h2>{intl.get('@MINI_BLOG.MINI-BLOG')}</h2>
                </a>
                <a hidden={sessionStorage.getItem('@userInfo.isBlogger')>0? false:true} href="#/miniblog/post/new" className="btn-post-creation">{intl.get('@MY_BLOG.POST-CREATION')}</a>
                ...
                <div className="row">
                  <div className="col-lg-9 col-md-8">
                    <div className="blog-details-main">
                        <Skeleton active avatar paragraph={{ rows: 6 }} loading={loading}>
                            <SelPost selPost={this.state.selPost} fileList={this.state.photoGallery} handleScoring={this.props.handleScoring} />
                        </Skeleton>
                    ...
                    <div className="blog-comments">
                        ...
                        <Skeleton active avatar paragraph={{ rows: 3 }} loading={loading}>
                        <div style={{ marginBottom: '40px' }}>
                            <SelPostComments postId={this.state.selPostId} showModal={this.showModal} handleCommentList={this.handleCommentList} handleCmntModify={this.handleCmntModify} />
                        </div>
                        {/* eslint-disable-next-line */}
                        <div className="add-comment" hidden={!hindCmntEditor||!hindCmntUpdater}><a className="btn-add-comment" onClick={()=>this.showModal(null, '')} >{intl.get('@MINI_BLOG.LEAVE-COMMENT')}</a></div>
                        <div id="blog-comment-ckform" style={{ height: '1em', width: '1em' }} />
                        </Skeleton>
                        <Modal 
                        ...
                        centered
                        footer={[
                            <Button key="back" onClick={this.handleCancel}>{intl.get('@GENERAL.BACK')}</Button>,
                            <Button key="confirm" type="primary" onClick={this.handleOk}>{intl.get('@GENERAL.AGREE')}</Button>
                        ]}
                        >
                        ...
                        >
                            <WrappedCommentForm hindCmntEditor={hindCmntEditor} quitCmntEditor = {this.handleCancelCmnt} handleCmntSubmit={this.handleCmntSubmit} postId = {this.state.selPostId} rootCmnt={this.state.rootCmnt} initReply={this.state.cmntAuthor} submitting={this.state.submitting} />                            
                        </div>
                        ...
                        >
                            <WrappedCommentEditForm hindCmntUpdater={hindCmntUpdater} selCmnt = {this.state.selCmnt} quitCmntEditor = {this.handleCancelCmntUpdate} handleCmntModifySubmit={this.handleCmntModifySubmit} editSubmitting={this.state.editSubmitting} />                            
                        </div>
                  ...
                  <div className="col-lg-3 col-md-4">
                    <div className="carousel vertical-slider" hidden={this.state.photoGallery.length===0? true:false}>
                        <SelPostGallery photo_gallery={this.state.photoGallery} />
                    </div>

                    <div className="related-resources">
                    <h2><span>{intl.get('@MINI_BLOG.RELATED-POSTS')}</span></h2>
                    
                    <ul className="related-blog-list related-list" style={{ padding: '15px 20px 15px 30px' }}>
                    <Skeleton active paragraph={{ rows: 3 }} loading={loading}>
                        {relatedPosts.length > 0?
                            relatedPosts.map((item,index)=>{
                                if(index<5){
                                    return(
                                        <li>
                                        {/* eslint-disable-next-line */}
                                        <a onClick={()=>this.handleRelated(item.id)} >{item.title}</a>
                                        <p><span className="date">{moment(item.publishAt).format('YYYY-MM-DD')}</span></p>
                            ...
                            }):<div style={{ width: '100%' }} ><Empty description={sessionStorage.getItem('lang')==='zh_TW'? '暫無相關文章':'No Related Post'} /></div>
                        }
                    </Skeleton>
                    </ul>
