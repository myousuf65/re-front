      ...
      this.props.form.validateFields((err,fieldsValues)=>{
            if(!getEData){
                ...
                const values = {
                    ...
                    'isReply2Cmnt': this.props.rootCmnt===null? 1:this.props.rootCmnt.id,
    ...
    render(){
        ...
        return(
            <Form style={{ display: 'block' }} onSubmit={this.handleSubmit} >
                ...
                { rules: [
                    {max: 500, message: sessionStorage.getItem('lang')==='zh_TW'? '應用更多格式可能會影響你的評論長度(建議少於150中文字)':'Content cannot be longer than 450 characters (html format considered).'}, 
                ...
                {/* ***s=]*** Replier Name */}
                <Form.Item
                label={sessionStorage.getItem('lang')==='zh_TW'? '回覆者姓名':'Name to be used as the replier'}
                // style={{ float: 'left', display: 'inline-block', width: '60%', minHeight:'80px' }}
                >
                {getFieldDecorator('isAlias',{ initialValue: this.state.selectedValue })(
                    <RadioGroup onChange={this.onRadioChange}>
                    <Radio style={radioStyle} value={0}>{this.state.username}</Radio>
                    <Radio style={radioStyle} value={1}>
                    {intl.get('@MY_BLOG.ALIAS')}
                        {this.state.selectedValue === 1? <Input style={{ width: 100, marginLeft:10 }} onChange={this.onRadioInputChange} allowClear /> : null}
                    </Radio>
                </RadioGroup>
                )}
                </Form.Item>
                ...
                <Form.Item>
                    ...
                    <Popconfirm title={sessionStorage.getItem('lang')==='zh_TW'? '你將放棄此次編輯，確定？':'Sure to quit and discard this draft?'} okText={sessionStorage.getItem('lang')==='zh_TW'?'是':'Yes'} cancelText={sessionStorage.getItem('lang')==='zh_TW'?'取消':'Cancel'} onConfirm={this.onPopconfirmOk}><Button style={{ marginLeft: '5%' }} shape="round">{intl.get('@MINI_BLOG.CANCEL')}</Button></Popconfirm>
                </Form.Item>
            </Form>
        )
    }
...
const WrappedForumCommentForm = Form.create({ name: 'comment_form' })(ForumCommentForm);
