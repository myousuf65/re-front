...
import { Popconfirm, Form, Icon, Input, Select, Switch, DatePicker, Radio, Button, message, Spin } from 'antd';
...
class PostForm extends Component{
    ...
    componentDidMount(){
        ...
        let getRecord_url = sessionStorage.getItem('serverPort')+'specialCollection/getDetail2/'+this.props.selPostId+'?user='+sessionStorage.getItem('@userInfo.id');
        fetchData(getRecord_url, 'get', null, response=>{
            if(response.ifSuccess){
              let res = response.result;
              if(res.status===200&&res.data.specialCollection && (res.data.specialCollection.createdBy===sessionStorage.getItem('@userInfo.id') ||res.data.specialCollection.originalCreator.id==sessionStorage.getItem('@userInfo.id') )){
                ...
                this.setState({ 
                    ...
                    disableTimePicker: (res.data.specialCollection.is_public===1? false : true),
                    // eslint-disable-next-line
                    disableAlias: (res.data.specialCollection.alias=="null"? false: true),
                    radioValue: (res.data.specialCollection.showAsAlias>0? "alias":"user"),
                    // eslint-disable-next-line
                    radioInput: (res.data.specialCollection.alias=="null"? null: res.data.specialCollection.alias),
                    loading: false
                });
    ...
    handleContentImg = (selPost) => {
        ...
        if(imgArr.length<1){
            this.props.form.setFieldsValue({ content: selPost.specialCollection.content });
        ...
        imgArr.forEach((img)=>{
            ...
            if(!imgOriginAlt.startsWith('resources/')&&imgOriginSrc){
                if(imgOriginSrc.startsWith('https://dsp.csd.hksarg/kms/api/')){
                    ...
                    if(imgId>-1){
                        let newKMSImg = `resources/${selPost.specialCollection.originalCreator.id}/${selPost.specialCollection.id}/${imgOriginSrc.slice(imgId+1)}`;
    ...
    handleSubmit = (e) => {
        ...
        if(this.props.form.getFieldValue('postTitle')){
            scanTarget = this.props.form.getFieldValue('postTitle') + ',' + scanTarget;
        }
        ...
        this.props.form.validateFields((err,fieldsValues)=>{
            if(fieldsValues['nominatedAuthor']==='alias'&&!radioInput){
                ...
                const values = {
                    ...
                    'alias': fieldsValues['nominatedAuthor']==='alias'? radioInput : this.state.selPost.specialCollection.alias,
                ...
                let updatePost_url = sessionStorage.getItem('serverPort')+'specialCollection/update/'+this.props.selPostId;
                fetchData(updatePost_url, 'post', values, response=>{
                    ...
                    if(response.ifSuccess){
                      ...
                      if(res.status===200){
                          ...
                          message.error("Failed to save. Your request was rejected by server.", 2);
                      }
                    }else{
                        message.error("Failed to save. Your request was rejected by server.", 2);
                    }
    ...
    onDelete = () => {
        window.location.assign(`#/specialCollection/details/?id=${this.props.selPostId}`);
    }
    ...
    render() {
        ...
        const { selPost, username, categoryList, mode, disableTimePicker, radioInput, radioValue, loading, disableAlias, isSuperAdmin, ruleOptions } = this.state;
        ...
        <div>
            ...
            <Form layout="horizontal" labelAlign="left" onSubmit={this.handleSubmit}>
                ...
                {getFieldDecorator('postTitle', {
                    rules: [{ required: true, message: intl.get('@GENERAL.TITLE-EN')+intl.get('@GENERAL.IS-REQUIRED') }],
                    initialValue: (selPost.specialCollection? selPost.specialCollection.postTitle:null)
                })(
                    <Input style={{ lineHeight: 0, maxWidth: '400px' }} prefix={<Icon type="file-text" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder={intl.get('@GENERAL.TITLE-EN')} allowClear />
                )}
                </Form.Item>
                ...
                {getFieldDecorator('postTitleZh', {
                    rules: [{ required: true, message: intl.get('@GENERAL.TITLE-ZH')+intl.get('@GENERAL.IS-REQUIRED') }],
                    initialValue: (selPost.specialCollection? selPost.specialCollection.postTitleZh:null)
                })(
                    <Input style={{ lineHeight: 0, maxWidth: '400px' }} prefix={<Icon type="file-text" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder={intl.get('@GENERAL.TITLE-ZH')} allowClear />
                )}
                </Form.Item>
                ...
                {getFieldDecorator('link', {
                    ...
                    initialValue: (selPost.specialCollection? selPost.specialCollection.link:null)
                })(
                    <Input disabled={isSuperAdmin?false:true} style={{ lineHeight: 0, maxWidth: '400px' }} prefix={<Icon type="file-text" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder={intl.get('@BOOKMARK.LINK')} allowClear />
                )}
                </Form.Item>  
                ...
                <Form.Item label={intl.get('@RES_DRAWER_INFO.ACCESS-CHANNEL')}>
                    {getFieldDecorator('accessChannel', { 
                        ...
                        initialValue: (selPost.specialCollection&&selPost.specialCollection.accessChannel? selPost.specialCollection.accessChannel:"1"),
                    })(
                        <Select>
                ...
                {getFieldDecorator('accessRuleId', { 
                    rules: [{required: true, message: intl.get('@GENERAL.REQUIRED')}],
                    initialValue: (selPost.specialCollection&&selPost.specialCollection.accessRuleId? selPost.specialCollection.accessRuleId:[]),
                })(
                ...
                <div style={{ marginBottom: '24px'}}>
                    ...
                    >
                    {getFieldDecorator('isPublic', { initialValue: selPost.specialCollection? !disableTimePicker:false, valuePropName: 'checked' })(
                    ...
                    help={publishAtError || (selPost.specialCollection===undefined? null : 'Last Publish at: '+moment(selPost.specialCollection.publishAt).format('YYYY/MM/DD HH:mm:ss'))}
                    >
                    {getFieldDecorator('publishAt',{initialValue: (selPost.specialCollection===undefined? moment() : moment(selPost.specialCollection.publishAt)), valueProName:'moment'})(
                        <DatePicker 
                        locale={locale}
                ...
                style={{display:'none'}}
                >
                {getFieldDecorator('nominatedAuthor',{ initialValue: selPost.specialCollection===undefined? "user": (selPost.specialCollection.showAsAlias>0?"alias":"user") })(
                    <RadioGroup name="nominatedAuthor" onChange={this.onRadioChange}>
                        <Radio style={radioStyle} value="user">{username}</Radio>
