...
class CommentEditForm extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps.selCmnt !== true &&
      nextProps.selCmnt !== this.props.selCmnt
    ) {
      console.log("comment_edit line 37");
  ...
  setContent = (cke_cmnt_ready, selCmnt) => {
    console.log("-----content-----", selCmnt.content, "------", cke_cmnt_ready);
  ...
  render() {
    ...
    return (
      <Form
        style={{ marginBottom: "75px", display: "block" }}
      ...
      >
        ...
        <Form.Item className="comment-form">
          {getFieldDecorator("content", {
            rules: [
              {
                ...
                message:
                  sessionStorage.getItem("lang") === "zh_TW"
                    ? "應用更多格式可能會影響你的評論長度(總長度300字以內)"
                    : "Content cannot be longer than 300 characters (html format considered).",
              },
            ...
            <CKEditor
              ...
              onInstanceReady={(evt) => {
                this.setState({ cke_cmnt_ready: true });
                this.setContent(true, this.props.selCmnt);
              }}
            />
          )}
        </Form.Item>
        ...
        >
          {getFieldDecorator("showAsAlias", {
            initialValue: this.props.selCmnt.showAsAlias,
          })(
            <RadioGroup onChange={this.onRadioChange}>
              <Radio style={radioStyle} value={0}>
                {this.props.selCmnt.createdBy === undefined
                  ? null
                  : this.props.selCmnt.createdBy.fullname}
              </Radio>
              <Radio style={radioStyle} value={1}>
                ...
                {this.state.selectedValue === -1 ? (
                  this.props.selCmnt.showAsAlias === 1 ? (
                    <Input
                      style={{ width: "8em", marginLeft: 10 }}
                      onChange={this.onRadioInputChange}
                      defaultValue={this.props.selCmnt.alias}
                      allowClear
                    />
                  ) : null
                ) : this.state.selectedValue === 1 ? (
        ...
        >
          <Button
            loading={this.state.editSubmitting}
            shape="round"
            htmlType="submit"
            type="primary"
          >
          ...
          >
            <Button style={{ marginLeft: "1em" }} shape="round">
...
const WrappedCommentEditForm = Form.create({ name: "comment_form_edit" })(
  CommentEditForm
);
