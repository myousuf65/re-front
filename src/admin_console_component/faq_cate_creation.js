...
import {Layout, Form, Input, Button, TreeSelect, Select, Icon, message, Popconfirm, Tooltip } from 'antd';
  ...
  handleSubmit=(e)=>{
    ...
    this.props.form.validateFieldsAndScroll((err, fieldsValues)=>{
      if(!err){
        ...
        var values = {
          ...fieldsValues,
          'titleTc': fieldsValues['titleTc']===undefined||fieldsValues['titleTc']===null||fieldsValues['titleTc']===''?fieldsValues['titleEn']:fieldsValues['titleTc'],
          'QuestionTc': fieldsValues['QuestionTc']===undefined||fieldsValues['QuestionTc']===null||fieldsValues['QuestionTc']===''?fieldsValues['QuestionEn']:fieldsValues['QuestionTc'],
          'AnswerTc': fieldsValues['AnswerTc']===undefined||fieldsValues['AnswerTc']===null||fieldsValues['AnswerTc']===''?fieldsValues['AnswerEn']:fieldsValues['AnswerTc'],
        ...
        let newCate_url = sessionStorage.getItem('serverPort')+'faq/create/'+sessionStorage.getItem('@userInfo.id');
      ...
      <Content className="cms-content" >
        <h1>
          ...
          <div style={{ display: 'inline-block', width: '40%', textAlign: 'right' }}>
              <Button className="res_create_btn" shape="round" type="primary" href="#/adminconsole/faq/management" ><Icon type="rollback" /> {intl.get("@RES_MANAGEMENT.BACK")}</Button>
          </div>
        </h1>
        ...
        <Form layout="vertical" labelCol={{ xs:{span:64}, sm:{span:6} }} wrapperCol={{ xs:{span:24}, sm:{span:18} }} labelAlign="left">
          ...
          <Form.Item label={intl.get('@FAQ_ADMIN.QUESTION-EN')}>
            {getFieldDecorator('QuestionEn')(
              <textarea   style={{whiteSpace: 'pre-wrap', minWidth: '500px'}} allowClear />
...
const WrappedFaqNewCateForm = Form.create({name: 'faq_new_cate_form'})(FaqNewCateForm);
