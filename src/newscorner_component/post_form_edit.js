...
class PostForm extends Component{
    ...
    componentDidMount(){
        ...
        let getRecord_url = sessionStorage.getItem('serverPort')+'blog/getDetail2/'+this.props.selPostId+'?user='+sessionStorage.getItem('@userInfo.id');
        fetchData(getRecord_url, 'get', null, response=>{
            if(response.ifSuccess){
              let res = response.result;
              if(res.status===200&&res.data.blog && (res.data.blog.createdBy===sessionStorage.getItem('@userInfo.id') ||res.data.blog.originalCreator.id==sessionStorage.getItem('@userInfo.id') )){
                ...
                this.setState({ 
                    ...
                    // eslint-disable-next-line
                    radioInput: (res.data.blog.alias=="null"? null: res.data.blog.alias),
                });
              }else{
        ...
        imgArr.forEach((img)=>{
            ...
            if(!imgOriginAlt.startsWith('resources/')&&imgOriginSrc){
                if(imgOriginSrc.startsWith('https://dsp.csd.hksarg/kms/api/')){
                    ...
                    if(imgId>-1){
                        let newKMSImg = `resources/${selPost.blog.originalCreator.id}/${selPost.blog.id}/${imgOriginSrc.slice(imgId+1)}`;
        ...
        this.props.form.validateFields((err,fieldsValues)=>{
            if(fieldsValues['nominatedAuthor']==='alias'&&!radioInput){
                ...
                const values = {
                    ...
                    'alias': fieldsValues['nominatedAuthor']==='alias'? radioInput : this.state.selPost.blog.alias,
                    
                };


                let updatePost_url = sessionStorage.getItem('serverPort')+'blog/update/'+this.props.selPostId;
    ...
    render() {
        ...
        const { selPost, username, categoryList, mode, disableTimePicker, radioInput, radioValue, loading, disableAlias, isSuperAdmin } = this.state;
        ...
        <div>
            ...
            <Form layout="horizontal" labelAlign="left" onSubmit={this.handleSubmit}>
                ...
                >
                    <Select className="postTags" disabled={true} value={selPost.blog? selPost.blog.categoryId:null} placeholder={intl.get('@MINI_BLOG.CATEGORY-TIPS')} >
...
{/* ---------------------------------------------------------------------------- */}
                ...
                {getFieldDecorator('videoLink', {
                    ...
                    <Input disabled={isSuperAdmin?false:true} style={{ lineHeight: 0, maxWidth: '600px' }} prefix={<Icon type="file-text" style={{ color: 'rgba(0,0,0,.25)' }} />} placeholder={intl.get('@MY_BLOG.VIDEO-LINK')} allowClear />
                )}
                </Form.Item>  
                ...
                <Form.Item
                label={intl.get('@MINI_BLOG.TAGS')}
                >
                {getFieldDecorator('tagList', {
                    initialValue: selPost.tagList? [...new Set(selPost.tagList)]:[],
                    rules: [{ type: 'array' }],
                })(
                    <Select className="postTags" mode="tags" placeholder={intl.get('@MINI_BLOG.TAGS-TIPS')} tokenSeparators={[';']} />
                )}
                </Form.Item>
                ...
                <div style={{ marginBottom: '24px'}}>
                    ...
                    >
                    {getFieldDecorator('isPublic', { initialValue: selPost.blog? !disableTimePicker:false, valuePropName: 'checked' })(
                    ...
                    help={publishAtError || (selPost.blog===undefined? null : 'Last Publish at: '+moment(selPost.blog.publishAt).format('YYYY/MM/DD HH:mm:ss'))}
                    >
                    {getFieldDecorator('publishAt',{initialValue: (selPost.blog===undefined? moment() : moment(selPost.blog.publishAt)), valueProName:'moment'})(
                        <DatePicker 
                        locale={locale}
                ...
                >
                {getFieldDecorator('nominatedAuthor',{ initialValue: selPost.blog===undefined? "user": (selPost.blog.showAsAlias>0?"alias":"user") })(
