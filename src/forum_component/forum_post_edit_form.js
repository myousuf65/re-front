      ...
      if (response.ifSuccess) {
        ...
        if (res.status === 200) {
          ...
          if (this.state.cke_ready) {
            this.handleContentImg(res.data, "content");
          }
          if (this.state.cke2_ready) {
            this.handleContentImg(res.data, "hiddenField");
          }
          ...
          this.setState({
            ...
            loadingPost: false,
            //   collecting: false,
            imgList: this.imgArrHandler(
              res.data.content + "<div>" + res.data.hiddenField + "</div>" ||
                "",
              "get"
            ),
            getPost_ready: true,
            ...
            galleryId:
              res.data.galleryId === undefined ? 0 : res.data.galleryId || 0,
          });
        } else {
          ...
          setTimeout(() => {
            window.location.assign(
              "#/kc/post/details?id=" + this.state.selPostId
            );
          }, 3000);
        }
      } else {
    ...
    this.props.form.validateFieldsAndScroll((err, fieldsValues) => {
      if (fieldsValues["nominatedAuthor"] === "alias" && !radioInput) {
        ...
        const values = {
          ...
          order:
            fieldsValues["order"] === undefined ||
            fieldsValues["order"] === null
              ? 999
              : fieldsValues["order"],
        };
          ...
          >
            ...
            <span hidden={selPost.canEdit < 1}>
              <Form.Item label={intl.get("@FORUM_ADMIN.STICKY")}>
                {getFieldDecorator("order", {
                  initialValue: selPost.order || 999,
                })(
                ...
                <Form.Item>
                  {getFieldDecorator("content", {
                    ...
                    <CKEditor
                      ...
                      onInstanceReady={(e) =>
                        this.onInstanceReady(e, "cke_ready")
                      }
                      onAfterSetData={(evt) => {
                  ...
                  <Form.Item>
                    {getFieldDecorator("hiddenField", {
                      ...
                      <CKEditor
                        ...
                        onPaste={this.onEditorPaste}
                        onInstanceReady={(e) =>
                          this.onInstanceReady(e, "cke2_ready")
                        }
                        onAfterSetData={(evt) => {
                          var data = evt.data.dataValue;
                          console.log("onAfterSetData: ", data);
                      ...
                      <List.Item
                        ...
                        extra={
                          <img
                            ...
                            name="auth-post-img"
                            onClick={(e) =>
                              this.handleImgSrc(item.url, e.target)
                            }
                            alt={item.url}
                            data={item.url}
            ...
            <Form.Item label={intl.get("@FORUM_GENERAL.ALIAS-AUTHOR")}>
              {getFieldDecorator("nominatedAuthor", {
                initialValue: selPost.isAlias ? "alias" : "user",
              })(
                <RadioGroup
...
const WrappedForumPostEditForm = Form.create({ name: "forum_post_edit_form" })(
  ForumPostEditForm
);
