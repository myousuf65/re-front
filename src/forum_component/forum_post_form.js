...
import { Popconfirm, Form, Icon, Input, Radio, Button, message, Breadcrumb, Upload, List, Checkbox } from 'antd';
    ...
    handleSubmit = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((err,fieldsValues)=>{
            if(fieldsValues['nominatedAuthor']==='alias'&&!radioInput){
                ...
                let createPost_url = sessionStorage.getItem('serverPort')+'forum/post/new/'+sessionStorage.getItem('@userInfo.id');
                fetchData(createPost_url, 'post', values, response=>{
                  if(response.ifSuccess){
                    let res = response.result;
                    if(res.status===200){
                        message.success("Post create successfully.");
                        setTimeout(()=> { window.location.replace(`#/kc/post/details?id=${res.data.id}`) }, 1000);
                    }else{
                ...
                {getFieldDecorator('nominatedAuthor',{ initialValue: "user" })(
                    <RadioGroup name="nominatedAuthor" onChange={this.onRadioChange}>
                        ...
                        <Radio style={radioStyle} value="alias">
                            ...
                            <Input onChange={this.onRadioInputChange} style={{ width: '8em', marginLeft:10, lineHeight: 0, display: 'inline-block' }} allowClear hidden={this.state.radioValue !== "alias"} />
                        </Radio>
                    </RadioGroup>
                )}
                </Form.Item>
...
{/* --------------------------------Submit or Quit-------------------------------------------- */}
                <Form.Item className="formButtons">
                    <Button type="primary" loading={loading} htmlType="submit">{intl.get('@GENERAL.CREATE')}</Button>
...
const WrappedForumPostForm = Form.create({ name: 'forum_post_form' })(ForumPostForm);
