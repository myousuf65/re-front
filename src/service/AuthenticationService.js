...
class AuthenticationService {
  ...
  executeJwtAuthenticationServiceForDP(clientHost) {
    return axios.post(
      `${sessionStorage.getItem("serverPort")}auth/generatetokendp`,
      {
        clientHost,
      }
    );
  ...
  isUserLoggedIn() {
    ...
    if (hostname === "dsp.csd.hksarg" && user === null) {
      //------for intranet
      this.executeJwtAuthenticationServiceForDP(hostname)
        .then((response) => {
          this.registerSuccessfulLoginForJwt(
    ...
    } else if (hostname === "dsptest.csd.ccgo.hksarg" && user === null) {
      //------for intranet
      this.executeJwtAuthenticationServiceForDP(hostname)
        .then((response) => {
          this.registerSuccessfulLoginForJwt(
